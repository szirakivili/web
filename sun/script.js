function getCoordinates() {
    return new Promise((resolve, reject) => {
        ...
        if (latitude && longitude) {
            // Coordinates found in URL, resolve immediately
            resolve({ lat: parseFloat(latitude), lon: parseFloat(longitude) });
        } else {
            // Coordinates not in URL, try GPS
            if (navigator.geolocation) {
                navigator.geolocation.getCurrentPosition(
                    ...
                    (error) => {
                        let errorMessage = "Nem sikerült a helymeghatározás";
                        switch (error.code) {
                        ...
                        // Return Budapest coordinates with the error message
                        alert(errorMessage + "\nA megjelenő adatok Budapesten érvényesek.");
                        resolve({ lat: 47.497885, lon: 19.040269 });
                    }
                );
            } else {
                const errorMessage = "Ez a böngésző nem támogatja a helymeghatározást.";
                // Reject if geolocation is not supported and display alert
                alert(errorMessage + "\nA megjelenő adatok Budapesten érvényesek.");
                resolve({ lat: 47.497885, lon: 19.040269 });
            }
        }
...
document.addEventListener('DOMContentLoaded', () => {
    getCoordinates()
        .then(coords => {
            // If coordinates are successfully retrieved (from URL or GPS)
            const apiUrl = `https://api.sunrisesunset.io/json?lat=${coords.lat}&lng=${coords.lon}&time_format=24`;
